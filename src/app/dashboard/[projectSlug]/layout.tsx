...
export default async function ProjectLayout({
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Tab nav */}
      <nav className="flex gap-1 border-b border-[#222] pb-0">
        {[
          ...
          <Link
            key={tab.label}
            href={tab.href}
            className="px-4 py-2 text-sm text-[#999] hover:text-violet-300 transition-colors border-b-2 border-transparent hover:border-violet-500/40 -mb-px"
          >
            {tab.label}
          </Link>
        ))}
