...
export const VoicePoweredOrb: FC<VoicePoweredOrbProps> = ({
  ...
  const externalIntensityRef = useRef(externalIntensity);

  // Keep ref in sync
  useEffect(() => {
    externalIntensityRef.current = externalIntensity;
  }, [externalIntensity]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      const update = (t: number) => {
        ...
        // Mic-based voice level
        if (enableVoiceControl && isMicrophoneInitialized) {
          voiceLevel = analyzeAudio();
          if (onVoiceDetected) {
            onVoiceDetected(voiceLevel > 0.1);
          }
        } else if (onVoiceDetected) {
          onVoiceDetected(false);
        }

        // Combine mic level with external intensity (e.g. agent speaking)
        const combinedLevel = Math.max(voiceLevel, externalIntensityRef.current);

        // Map combined level to rotation speed
        const rotationSpeed = baseRotationSpeed + (combinedLevel * maxRotationSpeed * 2.0);
